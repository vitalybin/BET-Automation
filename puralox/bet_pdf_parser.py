...
class BetPdfParser:
    """
    Parses a BET instrument PDF report and extracts all scientific data.

    All methods are static — no instance state is needed.
    Entry point: BetPdfParser.parse(pdf_source, out_dir)
    """

    # ── Class-level field pattern registry ─────────────────────────────
    ...
    def extract_highlight_strings(pdf_source: Union[str, Path, bytes]) -> List[str]:
        doc = BetPdfParser._open_pdf(pdf_source)
        ...
        for page_index in range(len(doc)):
            ...
            while annot:
                ...
                if subtype == fitz.PDF_ANNOT_HIGHLIGHT:
                    quad_rects = BetPdfParser._iter_quad_rects(annot)
                    captured = [
    ...
    def parse_highlights(highlights: List[str]) -> Dict[str, str]:
        ...
        expanded: List[str] = []
        for h in highlights:
            expanded.extend(BetPdfParser._expand_snippet(h))

        for snippet in expanded:
            matched = False
            for key, patterns in BetPdfParser.FIELD_PATTERNS:
                if key in data and len(data[key]) > 2:
                    continue
                for pat in patterns:
                    m = pat.search(snippet)
                    if m:
                        data.setdefault(key, BetPdfParser._norm_spaces(m.group(1)))
                        matched = True
                        break
                if matched:
                    break
            ...
            if not matched and ":" in snippet:
                ...
                if k and v and k not in data:
                    data[k] = BetPdfParser._norm_spaces(v)
    ...
    def parse_general(text: str) -> Dict[str, object]:
        d: Dict[str, object] = {}
        ops = [BetPdfParser._norm_spaces(o) for o in re.findall(r"\bOperator:\s*([^\n]+)", text, re.I)]
        ops = BetPdfParser._unique_preserve([o for o in ops if o])
        d["Operators"] = ops
        d["OperatorPrimary"] = ops[0] if ops else ""

        d["Dates"] = re.findall(
            r"\bDate[: ]\s*([0-9]{4}[/\-][0-9]{2}[/\-][0-9]{2}(?:\s[0-9:]+)?)", text, re.I
        )
        ...
        def grab(label: str, out_key: str | None = None):
            ...
            if m:
                d[key] = BetPdfParser._norm_spaces(m.group(1))

        for lab in [
            "Sample ID", "Filename", "Sample Desc", "Comment", "Sample weight",
            "Sample Volume", "Outgas Time", "OutgasTemp", "Analysis gas", "Bath Temp",
            "Press. Tolerance", "Equil time", "Equil timeout", "Analysis Time",
            "End of run", "Instrument", "Cell ID",
        ]:
    ...
    def parse_isotherm_summary(text: str) -> Dict[str, str]:
        blk = BetPdfParser._section(text, r"\bIsotherm\b", r"\bMBET summary\b|Page 2 of") or text
        out: Dict[str, str] = {}
        for key, pat in [
            ...
            if m:
                out[key] = BetPdfParser._norm_spaces(m.group(1))
        return out

    @staticmethod
    def parse_isotherm_points(text: str) -> pd.DataFrame | None:
        region = (
            BetPdfParser._section(text, r"Relative Pressure, P/Po", r"Isotherm\s*[\r\n ]*Slope")
            or BetPdfParser._section(text, r"Relative Pressure, P/Po", r"MBET summary|Page 2 of")
        )
        ...
        return df.sort_values("P_over_P0").reset_index(drop=True)

    @staticmethod
    def parse_multipoint_bet_summary(text: str) -> Dict[str, str]:
        blk = BetPdfParser._section(text, r"Multi-Point BET", r"t plot|t-plot|t plot") or text
        out: Dict[str, str] = {}
        for key, pat in [
    ...
    def parse_multipoint_bet_table(text: str) -> pd.DataFrame | None:
        region = BetPdfParser._section(text, r"Multi-Point BET Plot", r"Multi-Point BET|t plot|t-plot")
        if not region:
            return None
        ...
        return df.sort_values("P_over_P0").drop_duplicates("P_over_P0", keep="last").reset_index(drop=True)

    @staticmethod
    def parse_tplot_summary_and_points(text: str) -> tuple[Dict[str, str], pd.DataFrame | None]:
        blk = BetPdfParser._section(text, r"\bt plot\b|\bt-plot\b", r"BJH|BJH desorption") or text
        out: Dict[str, str] = {}
        for key, pat in [
        ...
        region = BetPdfParser._section(text, r"Statistical Thickness \(nm\)", r"BJH|BJH desorption") or blk
        vals = [float(x) for x in re.findall(r"[-+]?\d+(?:\.\d+)?(?:e[+-]?\d+)?", region, re.I)]
    ...
    def parse_bjh_table(text: str) -> pd.DataFrame | None:
        ...
        return df.sort_values("Diameter_nm").reset_index(drop=True)

    # ── Plot generators ─────────────────────────────────────────────────

    @staticmethod
    def _plot_isotherm(df: pd.DataFrame, out_png: Path) -> None:
        plt.figure()
    ...
    @staticmethod
    def _plot_tplot(df: pd.DataFrame, out_png: Path) -> None:
        plt.figure()
    ...
    @staticmethod
    def _plot_bjh(df: pd.DataFrame, out_png: Path) -> None:
        plt.figure()
    ...
    @staticmethod
    def _build_docx(bundle: Dict[str, object], dfs: Dict[str, pd.DataFrame],
                    plots: List[Path], out_docx: Path) -> None:
        if not HAVE_DOCX:
            return
        doc = Document()
    ...
    @staticmethod
    def parse(pdf_source: Union[str, Path, bytes],
              out_dir: Union[str, Path] = "bet_out",
              skip_docx: bool = True) -> Dict[str, object]:
        """
        Parse a BET PDF report and return a bundle dict with all extracted data.

        Args:
            pdf_source: path, Path, or raw bytes of the PDF file.
            out_dir:    directory to write CSV, PNG, and JSON outputs.
            skip_docx:  if False, also generates a .docx summary report.

        Returns:
            dict with keys: general, isotherm_summary, multipoint_bet_summary,
            tplot_summary, csvs, plots, docx.
        """
        out_dir = Path(out_dir)
        out_dir.mkdir(parents=True, exist_ok=True)

        doc  = BetPdfParser._open_pdf(pdf_source)
        text = BetPdfParser._full_text(doc)

        general      = BetPdfParser.parse_general(text)
        iso_summary  = BetPdfParser.parse_isotherm_summary(text)
        iso_points   = BetPdfParser.parse_isotherm_points(text)
        mp_summary   = BetPdfParser.parse_multipoint_bet_summary(text)
        mp_table     = BetPdfParser.parse_multipoint_bet_table(text)
        tp_summary, tp_points = BetPdfParser.parse_tplot_summary_and_points(text)
        bjh_table    = BetPdfParser.parse_bjh_table(text)
        ...
        if iso_points is not None and not iso_points.empty:
            p = out_dir / "isotherm.png";         BetPdfParser._plot_isotherm(iso_points, p); plots.append(p)
        if tp_points is not None and not tp_points.empty:
            p = out_dir / "tplot.png";            BetPdfParser._plot_tplot(tp_points, p);    plots.append(p)
        if bjh_table is not None and not bjh_table.empty:
            p = out_dir / "bjh_distribution.png"; BetPdfParser._plot_bjh(bjh_table, p);     plots.append(p)

        # Optional docx
        if (not skip_docx) and HAVE_DOCX:
            docx_path = out_dir / "BET_Full_Extract_Report.docx"
            BetPdfParser._build_docx(
                {"general": general, "isotherm_summary": iso_summary,
                 "multipoint_bet_summary": mp_summary, "tplot_summary": tp_summary},
                {"Isotherm points": iso_points, "Multi-Point BET table": mp_table,
                 "t-Plot points": tp_points,    "BJH Desorption table": bjh_table},
                plots, docx_path
            )
...
# PdfProcessor imports this name: `from .bet_integration import extract_all_with_prints`
# No changes needed in pdf_processor.py.
extract_all_with_prints = BetPdfParser.parse
